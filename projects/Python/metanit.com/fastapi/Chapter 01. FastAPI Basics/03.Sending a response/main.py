...
# В прошлой теме рассматривалась обработка запроса функциями, например, простейший код:
...
# Однако функция необязательно должна возвращать именно данные в формает json.
# В реальности мы можем возвратить объект класса Response или одного из его
# подклассов (коим также является JSONResponse), которые позволяют отправлять
# клиенту ответ в различных видах и формах. Рассмотрим некоторые из этих классов.

# RESPONSE

# Класс fastapi.Response является базовым для остальных классов ответа. Его преимуществом
# является то, что он позволяется также отправить ответ, который не покрывается
# встроенными классами, например, в каком-то нестандартном формате. Для отпределения
# ответа конструктор класса принимает следующие параметры:
...
# В данном случае клиенту отправляет обычная строка "Hello METANIT.COM".
# А MIME-тип "text/plain" указывает, что тип ответа - простой текст.
...
# Для упрощения отправки кода html предназначен класс HTMLResponse.
...
# Методы FastAPI такие как get(), post() и т.д. позволяют
# задать тип ответа с помощью параметра response_class:
...
# Добавьте этот эндпоинт для корневого URL
@app.get("/")
def root():
    return {"message": "Go to /text or /html"}
