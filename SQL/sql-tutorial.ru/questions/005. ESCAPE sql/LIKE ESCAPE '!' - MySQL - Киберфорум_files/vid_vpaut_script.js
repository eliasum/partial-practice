...
function Vid_banner_listener(e) {
    if (is_mob) {
        if (e.data == 'surok_check_banner_mob') {
            ...
            let vpaut_pl = (el.hasAttribute('vid_vpaut_pl') && parseInt(el.getAttribute('vid_vpaut_pl')) > 0) ? parseInt(el.getAttribute('vid_vpaut_pl')) : 0;
            if (!vpaut_pl) {
                console.info('Vid vpaut no pl');
                return;
            }
            /* let frametm = document.getElementById('tmp_div_vid_vpaut');
             GetBanner(frametm, vpaut_pl, count_mob).catch(error => {
...
function Vid_vpaut_listener(e) {
        ...
        try {
            if (ind < adlen) {
                ...
                if (overload && endless_mob > 0) {
                    ...
                    /*  GetBanner(frametm, vpautPl, 0).catch(error => {
                          console.info('VID ' + error.message);});*/
                    /*   clickMobKrestik();
    ...
    if (e.data == 'surok_check_reklamend' || e.data == 'surok_no_reklam') {
        try {
            if (ind < adlen) {
                if (!has_ad) {// && is_cap
                    let adv_vpaut_id = document.getElementById('vid_vpaut_kod_frame');
                    // adv_vpaut_id.src = giveAdvHref(adv_reklamn[ind]);
                    adv_vpaut_id.contentWindow.location.replace(giveAdvHref(adv_reklamn[ind]));
                    //console.info(adv_vpaut_id.src + ' VPAUT HREFS');
                    ind++;
                } else {//if (!is_cap)
                    let adv_vpaut_id = document.getElementById('vid_vpaut_kod_frame');
                    // adv_vpaut_id.src = giveAdvHref(adv_reklamn[ind]);
    ...
    if (e.data == 'surok_complete_adv_mob') {
        try {
            /* let delayMobAdv = setTimeout(function () {
                 if(!checkShowMobile){
                     let frame = document.getElementById('tmp_div_vid_vpaut');
                     frame.style.height='2px';
                 } else checkShowMobile = 0;
                 clearTimeout(delayMobAdv);
                 delayMobAdv = null;
             }, 2000);
             checkShowMobile = 0;*/
...
/*function giveAdvHref(href) {
    if (href.substr(-4, 4) == '?pl=') {
        if (flyroll_player) {
            return href + '' + vpaut_pl + '&ra=1';
        }
        return href + '' + vpaut_pl;
    }
    return href;
}*/
...
function GetBanner(el, plId, countB) {
    return fetch('https://videoroll.net/banners/index.php?id=' + plId + '&count=' + countB + '&countryp=' + countryp, {
        ...
        banner => {
            // console.info('https://videoroll.net/banners/index.php?id='+plId+'&count='+countB+'&countryp='+countryp);
            if (banner != 'END') {
                el.innerHTML = banner;
                BannerKrestik(el);
               // closeBnner();
...
function GetOptionVidVpaut(el, pl_id) {
    return fetch('https://videoroll.net/vpaut_option_get.php?pl_id=' + pl_id)
        .then(
            ...
            tmp_div_vid_vpaut.style.zIndex = 99000;
            if (!is_mob) {
                ...
                height_roll = (el.hasAttribute('vid_roll_height') && parseInt(el.getAttribute('vid_roll_height')) > 248) ? parseInt(el.getAttribute('vid_roll_height')) : 248;
                ...
                tmp_div_vid_vpaut.style.left = 0;
            }
            ...
            //console.info(iframeAdElem.src + ' VPAUT HREFS');
            ...
            // if(is_mob)ShowKrestic(tmp_div_vid_vpaut);
...
function Check_Vid_Vpaut_Visibility() {
    if (is_open_vid_vpaut && change_krestic && !is_mob) {
        ...
        if (trigger_y > topD && trigger_y < bottomD && !only_fly) {
            ...
            if (param.hasAttribute('vid_el_targent')) {
                if (document.getElementById(param.getAttribute('vid_el_targent'))) {
                    ...
                    if (adv_overs && width_roll < 480) {
                        left_el_rect = parseInt(document.getElementsByClassName(param.getAttribute('vid_el_targent'))[0].getBoundingClientRect().left) + 240;
            ...
            if (param.hasAttribute('vid_el_targent_r')) {
                if (document.getElementById(param.getAttribute('vid_el_targent_r'))) {
                    if (adv_overs && width_roll < 480) {
                        left_el_rect = parseInt(document.getElementById(param.getAttribute('vid_el_targent_r')).getBoundingClientRect().left) - 480;
                    ...
                    if (adv_overs && width_roll < 480) {
                        left_el_rect = parseInt(document.getElementsByClassName(param.getAttribute('vid_el_targent_r'))[0].getBoundingClientRect().left) - 480;
                    } else {
            ...
            if (param.hasAttribute('vid_el_right_targent_r')) {
                if (document.getElementById(param.getAttribute('vid_el_right_targent_r'))) {
                    if (adv_overs && width_roll < 480) {
                        left_el_rect = parseInt(document.getElementById(param.getAttribute('vid_el_right_targent_r')).getBoundingClientRect().right) - 480;
                    ...
                    if (adv_overs && width_roll < 480) {
                        left_el_rect = parseInt(document.getElementsByClassName(param.getAttribute('vid_el_right_targent_r'))[0].getBoundingClientRect().right) - 480;
        ...
        try {
            document.getElementById("vid_vpaut_kod_frame").style.width = "100% !important";
        } catch (b) {
        }
        try {
...
function Vid_vpaut_onload() {
    ...
    GetOptionVidVpaut(el, vpaut_pl).catch(error => {
        console.info('VID ' + error.message);
